        ...
        function releaseWakeLock() {
         ...
         function parseSRTData(data) {
             ...
             blocks.forEach(block => {
                 ...
                 // This accepts valid SRT variants with/without cue numbers and
                 // avoids silently dropping subtitles from slightly non-standard files.
                 ...
                 if (text) {
                     videoSubtitles.push({ start, end, text });
                     sentences.push(text);
                 }
             });
         }
         ...
         function renderPlaylist() { 
             ...
             if (sentences.length > 0) { 
                 ...
                 sentences.forEach((sentence, index) => { 
                     ...
                     wrapper.id = `playlist-item-${index}`; 
                     if (hiddenSentences.has(sentence)) wrapper.classList.add('is-hidden'); 
                     const contentDiv = document.createElement('div'); contentDiv.className = 'sentence-content'; 
                     const topRow = document.createElement('div'); topRow.className = 'sentence-top-row'; 
                     ...
                     textSpan.innerText = `${index + 1}. ${translationState[sentence] === getTranslationLanguage() && selectedTranslation ? selectedTranslation : sentence}`; 
                 ...
                 // Keep Sentence Index/tools visible even before a box is created or loaded.
                 playlistSection.style.display = 'block'; 
             }
         }
         ...
         document.getElementById('actGptBtn').addEventListener('click', () => {
             if (sentences.length === 0 || sentenceIndex === -1) return;
             const text = sentences[sentenceIndex];
             if (navigator.clipboard) navigator.clipboard.writeText(text).catch(() => fallbackCopyTextToClipboard(text));
             else fallbackCopyTextToClipboard(text);
             const encoded = encodeURIComponent(text);
             const isAndroid = /android/i.test(navigator.userAgent);
             if (isAndroid) window.location.href = `intent://chat.openai.com/?q=${encoded}#Intent;scheme=https;package=com.openai.chatgpt;action=android.intent.action.VIEW;end`;
             else window.open(`https://chat.openai.com/?q=${encoded}`, '_blank');
         });
         ...
         function highlightActivePlaylistSentence() { document.querySelectorAll('.sentence-content').forEach(el => el.classList.remove('active')); if (sentences.length > 0 && sentenceIndex !== -1 && sentenceIndex < sentences.length) { const activeWrapper = document.getElementById(`playlist-item-${sentenceIndex}`); if (activeWrapper) { const activeContent = activeWrapper.querySelector('.sentence-content'); if(activeContent) activeContent.classList.add('active'); activeWrapper.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } highlightInBoxEditor(); } }
         ...
         function updateVisibilityStates() { 
             ...
             document.querySelectorAll('.sentence-wrapper').forEach((el, index) => { 
                 ...
                 if (index < startSentenceIndex || index > endSentenceIndex || (playStarredOnlyInput.checked && !starredSentences.has(sentences[index]))) outOfRange = true;
                 if (outOfRange) el.classList.add('out-of-range'); else el.classList.remove('out-of-range'); 
                 let count = getWordCount(sentences[index]); let failWords = false;
                 if (!isNaN(minW) && minW > 0 && count < minW) failWords = true;
                 if (!isNaN(maxW) && count > maxW) failWords = true;
                 
                 let failSearch = false;
                 if (searchVal && !sentences[index].toLowerCase().includes(searchVal)) failSearch = true;
         ...
         window.addEventListener('DOMContentLoaded', () => { 
             loadSettings(); loadFS(); renderFS(); updateNowPlaying(); 
         ...
         function updateScreenOnly() { 
             ...
             currentSentenceDiv.innerHTML = renderSentenceWithWords(sentenceStr);
              stopPauseCountdown(); 
         ...
         function updateVideoCurrentSubtitleIndicator() {
             ...
             if (activeIndex !== -1) {
                 ...
                 const activeWrapper = document.getElementById(`playlist-item-${activeIndex}`);
                 if (activeWrapper) {
         ...
         function speakNext() {
             ...
             currentSentenceDiv.innerHTML = getPlaybackGroupDisplayHtml(groupIndices);
             highlightActivePlaylistSentence();
             updateActiveSentenceToolbar();
...
(function(){
  ...
  installBtn.id = 'installAppBtn'; installBtn.type = 'button';
  installBtn.textContent = '📲 Install App'; installBtn.hidden = false;
  document.addEventListener('DOMContentLoaded', function(){ document.body.appendChild(installBtn); });
